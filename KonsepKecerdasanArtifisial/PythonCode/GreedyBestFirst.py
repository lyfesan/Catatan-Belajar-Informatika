# Implementasi Greedy Best First Search untuk rute Ponorogo -> Surabaya
...
class Graph:
    ...
    def greedyBestFirst(self ,s ,dest):
        cost = 0
        queue = []
        queue.append(s)
        visited = set() 
        ...
        while queue: 
            ...
            #Evaluasi rute dengan f(x) = h(x)
            for i,j in self.graph[s]:
                if self.hx[i] < costTmp[2]:
...
print("Greedy Best First Search Algorithm from Ponorogo to Surabaya:")
g.greedyBestFirst("Ponorogo", "Surabaya")